...
function partner($db){
	...
	if(isset($_POST["addNew"])) {
        $insert = array('title'=>$title,'ind'=>$ind,'active'=>$active,'facebook'=>$facebook);
		try{
            $recent = $db->insert($table,$insert);
            if(common::file_check($_FILES['file'])){
                WideImage::load('file')->resize(100,100,'fill')->saveToFile(myPath.$file);
                $db->where('id',$recent);
	...
	if(isset($_POST["update"]))	{
        $update=array('title'=>$title,'ind'=>$ind,'active'=>$active,'facebook'=>$facebook);
        if(common::file_check($_FILES['file'])){
            WideImage::load('file')->resize(100,100, 'fill')->saveToFile(myPath.$file);
    ...
    if($count!=0){
        foreach($list as $item){
            $img=($item['img']=='')?selfPath.'square-facebook.png':myPath.$item['img'];
            $item_content = array(                
                array($img,'image'),                
                array($item['title'],'text'),
                array($item['facebook'],'link'),
                array($item['ind'],'text'),
	...
	<div class="row">
        ...
        <div class="col-lg-12">
            ...
            '.$form->text('facebook',array('label'=>'Liên kết<code>http://facebook.com/person</code>')).'
            '.$form->file('file',array('label'=>'Hình ảnh <code>( đề nghị: 100,100 )</code>')).'
