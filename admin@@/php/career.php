...
function career($db)
{
    ...
    $page_head= array(
                    array('#','Danh sách tuyển dụng')
                );
	$str=$form->breadcumb($page_head);
	$str.=$form->message($msg);
    
    $str.=$form->search_area($db,$act,'career_cate',$_GET['hint'],0);
    ...
    if($db->count!=0){
        foreach($list as $item){
            //$cate=$db->where('id',$item['pId'])->getOne('career_cate','id,title');
            $item_content = array(
        ...
        <div class="col-lg-12 admin-tabs">
    		...
    		<div class="tab-content">
    			<div class="tab-pane bg-vi active" id="vietnamese">
                    ...
                    '.$form->textarea('meta_description',array('label'=>'Meta Description<code>SEO</code>')).'   
    			...
    			<div class="tab-pane bg-en" id="english">
                    ...
                    '.$form->textarea('e_meta_description',array('label'=>'Meta Description<code>SEO</code>')).'   
